...
const ForgotPassword = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!email) {
      toast.error(t('auth.enterEmail') || 'Please enter your email')
      return
    }
    ...
    return (
      <div className="min-h-screen    flex items-center justify-center px-4 pt-24">
        <div className="max-w-md w-full">
          <div className=" p-8 rounded-xl border  shadow-2xl text-center">
            <div className="w-16 h-16  rounded-full flex items-center justify-center mx-auto mb-4 shadow-lg">
              <span className="text-2xl">✅</span>
            </div>
            <h2 className="text-gray-900 dark:text-gray-900 dark:text-gray-900 dark:text-gray-900 dark:text-white font-bold text-2xl mb-3">{t('auth.checkEmail') || 'Check Your Email'}</h2>
            <p className="text-gray-600 dark:text-gray-600 dark:text-gray-600 dark:text-gray-600 dark:text-gray-300 mb-6">
              {t('auth.resetSentTo') || "We've sent a password reset link to"} <span className="font-semibold text-blue-400">{email}</span>
            </p>
            <div className="bg-blue-900/30 border border-blue-500/30 rounded-lg p-4 mb-6">
              <p className="text-sm text-blue-300">
                ⏱️ {t('auth.linkExpires')}
              </p>
            </div>
            <p className="text-gray-600 dark:text-gray-600 dark:text-gray-600 dark:text-gray-600 dark:text-gray-300 text-sm mb-6">
              {t('auth.linkOpenPage') || 'The link will open a page where you can create a new password.'}
            </p>
            <button
        ...
        <div className="bg-white dark:bg-gradient-to-br dark:from-slate-800 dark:to-slate-900 p-8 rounded-xl border border-gray-200 dark:border-gray-200 dark:border-gray-200 dark:border-gray-200 dark:border-slate-700 shadow-2xl">
          <div className="text-center mb-8">
            ...
            <p className="text-gray-600 dark:text-gray-600 dark:text-gray-600 dark:text-gray-600 dark:text-gray-300">
              {t('auth.forgotMessage') || 'Enter your email address and we\'ll send you a link to reset your password.'}
            </p>
          </div>
