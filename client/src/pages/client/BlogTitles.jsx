...
const BlogTitles = () => {
  ...
  const onSubmitHandler = async (e)=> {
    ...
    try {
      setLoading(true)
      const uiPrompt = `Generate a blog title for the keyword ${input} in the category ${selectedCategory}`

      const aiPrompt = `Generate 5 SEO-friendly blog titles for the keyword "${input}" in the ${selectedCategory} category.

Format:
Here are 5 SEO-friendly blog titles for the keyword "${input}":
  ...
  return (
    <div className='h-full overflow-y-scroll p-6 flex items-start flex-wrap gap-4 text-slate-700'>
      ...
      <form onSubmit={onSubmitHandler} className='w-full max-w-lg p-4 bg-white rounded-lg border border-gray-200'>
        ...
        <br />
        <button disabled={loading} className='w-full flex justify-center items-center gap-2 bg-linear-to-r from-[#C341f6] to-[#8E37EB] text-gray-900 dark:text-gray-900 dark:text-gray-900 dark:text-gray-900 dark:text-white px-4 py-2 mt-6 text-sm rounded-lg cursor-pointer'>
          {
