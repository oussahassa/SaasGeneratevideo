...
export const generateVideoScript = async (req, res) => {
  try {
    ...
    if (plan !== "premium" && free_usage >= 5) {
      return res.json({
        success: false,
        message: "Video generation limit reached. Upgrade to premium.",
      });
    }
    ...
    await sql`
      INSERT INTO videos (id, user_id, script, status, type)
      VALUES (${videoId}, ${userId}, ${script}, 'script_generated', 'generated')
    `;
...
export const generateVideoFromAssets = async (req, res) => {
  try {
    ...
    await sql`
      UPDATE videos 
      SET status = 'completed', video_url = ${videoUrl}
      WHERE id = ${videoId}
    `;
